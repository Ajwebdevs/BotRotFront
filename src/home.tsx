...
function Home() {
  ...
  const speakText = (text) => {
    if (text && voices.length > 0) {
      const sentences = text.match(/[^\.!\?]+[\.!\?]+|[^\.!\?]+$/g) || [text]; // Split into sentences or fallback to the full text
      let index = 0;
  ...
  // Function to handle TTS with random pitch and rate
//   const speakText = (text) => {
//     if (text && voices.length > 0) {
...
//             utterance.voice = voices[0]; // Fallback to the first available voice
//           }
//           // Random pitch and rate between 0.6 and 0.9
//         //   utterance.pitch = 0.6 + Math.random() * 0.3;
//           utterance.pitch = 0.8;
//           utterance.rate = 0.8;
//           speechSynthesis.speak(utterance);
//         }
//         //   utterance.rate = 0.6 + Math.random() * 0.3;
//         //   utterance.onend = () => {
//         //     index++;
//         //     speakNextSentence();
//         //   };
...
//   };
  ...
  return (
    ...
    <TooltipProvider delayDuration={200}>
      ...
      <div className="relative flex flex-col items-center justify-center min-h-screen bg-transparent">
        ...
        <TypographyH1 className="text-center text-white z-10 mb-12">
          This here lil' buddy turns ur boring normie English 🥱 into some chaotic
          z or brainrot lingo 🤪. Wanna confuse ur frens? Type away!
        </TypographyH1>
        {/* Goofy Brain Icon */}
        <img
    ...
    {/* Animation Styles */}
    <style jsx global>{`
      @import url('https://fonts.googleapis.com/css2?family=Glitch+Goblin&display=swap');
